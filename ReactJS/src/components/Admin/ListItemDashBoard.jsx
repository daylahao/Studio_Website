...
function ListItemDashBoard(props) {
    ...
    const items =(data,stt)=>{
        return  <tr className='text-center h-100' style={{ cursor:'pointer' }}>
        <td  onClick={()=>{
        ...
        <td  onClick={()=>{
            navigate('/dashboard?action=products&&id='+data.id_item);
            window.location.reload();
        }} className='d-none d-md-table-cell justify-content-center'><Image src={data.image} rounded width={170}></Image></td>
        <td  onClick={()=>{
            navigate('/dashboard?action=products&&id='+data.id_item);
            window.location.reload();
        }}>{data.name}</td>
        <td  onClick={()=>{
            navigate('/dashboard?action=products&&id='+data.id_item);
            window.location.reload();
        }} className='d-none d-md-table-cell text-center h-100'>{data.description}</td>
        <td  onClick={()=>{
            navigate('/dashboard?action=products&&id='+data.id_item);
            window.location.reload();
        }}>{data.price}</td>
    ...
    if(dataitem){
        return(
            <div className='p-3 d-flex flex-column overflow-hidden' style={{ height:'70vh',color:'var(--black)'}}>
             <header className='w-100 align-items-end d-flex justify-content-end gap-3 my-2'>
                <Button variant='outline-success px-3' onClick={()=>{
                                    navigate('/dashboard?action=products');
                                    window.location.reload();
                }}><i class="bi bi-arrow-return-left"></i> Quay lại</Button>
             </header>
                <Row>
                    <Col>
                        <h2>Hình ảnh</h2>
             ...
             <Row className='h-100 overflow-hidden d-flex flex-row w-100'>
                    <Col xs={4} className=' d-flex align-items-center overflow-hidden '>
                        ...
                        <div id='avatarThumbDashBoardChange' className=' h-100 position-relative ' style={{width:'100%'}}
                        onClick={()=>{
                            document.getElementById('importthumbnail').click();
                        }}
                        >
                        <Image id="" src={file?(file):(dataitem.image)} className='w-100 position-absolute translate-middle-y' style={{ paddingTop:'100%'}} ></Image>
                        <p id='labelavatarthumbdashboardchange' className='position-absolute translate-middle-y w-100 text-center h-100 top-0 bottom-0' >Thay Đổi Ảnh</p>
                        <input id='importthumbnail' type='file' className='w-100 d-none' onChange={handleChange} name='importthumbnail'></input>
                        </div>
                        </div>
                    </Col>
                    <Col xs={8} className='gap-2 d-flex flex-column'>
                    <label>Tên</label>
                        <textarea value={dataitem.name} className='w-100 form-control' maxLength={60}  style={{resize: 'none'}} onChange={(e)=>{
                            setDataItem({...dataitem,name:e.target.value});
                        }}></textarea>
                        <label>mô tả</label>
                        <textarea value={dataitem.description} className='w-100 form-control' maxLength={60}  style={{resize: 'none'}} onChange={(e)=>{
                            setDataItem({...dataitem,description:e.target.value});
                        }}></textarea>
                            <label>Giá</label>
                        <input className='form-control' value={dataitem.price} onChange={(e)=>{
                            setDataItem({...dataitem,price:e.target.value});
                        }}></input>
                        <label>Loại thiết bị</label>
                        <select class="form-select" aria-label="Default select example" onClick={LoadOption}
                        onSelect={(e)=>{
                            setDataItem({...dataitem,type:listOptionType.find((item)=>item.id==e.target.value),id:e.target.value});
                        }}
                        onChange={(e)=>{
                            setDataItem({...dataitem,type:listOptionType.find((item)=>item.id==e.target.value).type,id:e.target.value});
                        }}
                        >
                        <option selected>{dataitem.type}</option>
                        {listOptionType?(listOptionType.map((item)=>{
                ...
                <Row className='w-100 align-items-end d-flex justify-content-end gap-3 my-2 '>
                    ...
                    <Button variant='success' className='w-100' onClick={handleUpadteItem}>Lưu</Button>
                    </Col>
                    <Col xs={12} md={2} className='justify-content-end d-flex'>
    ...
    return (
        <div className='p-3 d-flex flex-column overflow-hidden ' style={{ height:'70vh'}}>
            <header className='w-100 align-items-end d-flex justify-content-end gap-3 my-2'>
                <Button variant='success px-3' onClick={()=>{
                ...
                <div className='d-flex h-100 gap-2 mx-2'>
                    ...
                    <Button className='btn-success rounded-circle m-auto'><i class="bi bi-search"></i></Button>
                </div>
            </header>
